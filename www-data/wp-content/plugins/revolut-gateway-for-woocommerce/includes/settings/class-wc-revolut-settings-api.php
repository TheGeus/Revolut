...
class WC_Revolut_Settings_API extends WC_Settings_API {
	...
	public function maybe_register_webhook() {
		...
		if ( ! $this->setup_revolut_webhook() ) {
			/* translators:%1s: %$2s: */
			$this->add_error_message( sprintf( __( 'Webhook setup unsuccessful. Please make sure you are using the correct %1$sAPI key%2$s. If the problem persists, please reach out to support via our in-app chat.', 'revolut-gateway-for-woocommerce' ), '<a href="https://developer.revolut.com/docs/accept-payments/get-started/generate-the-api-key" target="_blank">', '</a>' ) );
			return false;
		}
